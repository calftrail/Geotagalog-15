...
%Image::ExifTool::RIFF::Info = (
    ...
    NOTES => q{
        ...
        of the EXIF 2.2 specification (Tag ID's beginning with "I") have an
        underlined Tag Name in the HTML version of this documentation.  Other tags
        are found in AVI files generated by Sony Vegas video editing software.
    },
    ...
    ISRF => 'SourceForm',
    ITCH => 'Technician',
    # the following tags (TAPE->DTIM) are Sony Vegas AVI tags, and are
    # also used by SCLive and Adobe Premier (ref 11)
    TAPE => {
        Name => 'TapeName',
        Groups => { 2 => 'Video' },
    },
...
%Image::ExifTool::RIFF::Exif = (
    PROCESS_PROC => \&Image::ExifTool::RIFF::ProcessChunks,
    GROUPS => { 2 => 'Audio' },
    NOTES => 'These tags are part of the EXIF 2.2 specification for WAV audio files.',
    ever => 'ExifVersion',
...
%Image::ExifTool::RIFF::Composite = (
    ...
    Duration2 => {
        ...
        RawConv => '($val[0] and not $val[2]) ? $val[1] / $val[0] : undef',
        PrintConv => 'ConvertDuration($val)',
    },
);
        ...
        if ($tagInfo) {
            if ($$tagInfo{SubDirectory}) {
                ...
                if (defined $newBase) {
                    ...
                    $baseShift = $newBase - $$dirInfo{Base};
                    $start -= $base;
                }
