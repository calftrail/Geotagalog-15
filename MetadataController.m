...
@interface TLPhotoSourceItem (MetadataControllerAdditions)
@property (nonatomic, readonly) BOOL metadataControllerCanGeotag;
@end
...
@property (nonatomic, readonly) id geotaggedCollection;
@end
					   ...
					   context:(void*)context
{
	...
	if (didLocatePhotos) {
		...
		BOOL ungeotaggables = [[[self itemSource] items] count] > [[self geotaggedCollection] count];
...
- (id)geotaggedCollection {
	BOOL rejectInaccurateLocations = [[NSUserDefaults standardUserDefaults]
									  boolForKey:@"LimitInaccurateGeotagging"];
	return rejectInaccurateLocations ? (id)accurateItems : (id)itemPhotos;
}

- (NSMapTable*)itemMetadata {
	NSMapTable* itemMetadata = [NSMapTable mapTableWithStrongToStrongObjects];
	for (TLPhotoSourceItem* item in [self geotaggedCollection]) {
		TLPhoto* photo = [itemPhotos objectForKey:item];
		NSDictionary* metadata = [NSDictionary dictionaryWithObjectsAndKeys:
								  [photo location], TLMetadataLocationKey,
								  [photo timestamp], TLMetadataTimestampKey,
								  [photo timeZone], TLMetadataTimezoneKey, nil];
					...
					timeZone:(NSTimeZone*)timeZone
{
	...
	for (TLPhotoSourceItem* photoItem in items) {
		TLTimestamp* originalTimestamp = [[photoItem metadata] objectForKey:TLMetadataTimestampKey];
		if (![photoItem metadataControllerCanGeotag]) {
			[itemPhotos removeObjectForKey:photoItem];
			[accurateItems removeObject:photoItem];
			continue;
		}
		NSDate* errorCorrectedDate = [[originalTimestamp time] addTimeInterval:error];
		NSInteger timeZoneOffset = -[timeZone secondsFromGMTForDate:errorCorrectedDate];
		NSDate* adjustedDate = [errorCorrectedDate addTimeInterval:(NSTimeInterval)timeZoneOffset];
		TLTimestamp* timestamp = [TLTimestamp timestampWithTime:adjustedDate
...
- (void)updateExportReady {
	...
	BOOL haveItemsForExport = ([[self geotaggedCollection] count] ||
...
@implementation TLPhotoSourceItem (MetadataControllerAdditions)

- (BOOL)metadataControllerCanGeotag {
	return ([self canGeotag] && [[self metadata] objectForKey:TLMetadataTimestampKey]);
}
