...
- (void)acceptItem:(TLFileSourceItem*)newItem {
	...
	if (![self isEngaged]) {
		...
		[[self allPaths] addObject:[newItem filePath]];
	}
	else {
		...
		NSError* fullError = [NSError errorWithDomain:@"com.calftrail.tagalog"
												 code:1000
											 userInfo:errInfo];
...
- (void)addSuboperationsForFolder:(NSString*)folderPath {
	...
	NSArray* filenames = [[NSFileManager tlThreadManager] contentsOfDirectoryAtPath:folderPath
																			  error:NULL];
...
- (void)addItemForPath:(NSString*)itemPath {
	...
	BOOL isDirectory = NO;
	BOOL exists = [[NSFileManager tlThreadManager] fileExistsAtPath:[self path]
