...
static NSString* const TLTagalogCameraErrorDefault = @"CameraError";
static NSString* const TLTagalogManualTimeZoneDefault = @"UseManualTimeZone";
static NSString* const TLTagalogManualOffsetDefault = @"ManualTimeZoneSecondsOffset";
...
- (void)setCameraError:(NSTimeInterval)newCameraError {
	...
	[[NSUserDefaults standardUserDefaults]
	 setObject:[NSNumber numberWithDouble:newCameraError] forKey:TLTagalogCameraErrorDefault];
}
...
- (void)setManualTimeZone:(BOOL)newManualTimeZone {
	...
	[[NSUserDefaults standardUserDefaults]
	 setBool:newManualTimeZone forKey:TLTagalogManualTimeZoneDefault];
}
...
- (void)setManualTimeZoneSecondsOffset:(NSInteger)newManualTimeZoneSecondsOffset {
	...
	[[NSUserDefaults standardUserDefaults]
	 setInteger:newManualTimeZoneSecondsOffset forKey:TLTagalogManualOffsetDefault];
}
...
- (void)awakeFromNib {
	...
	BOOL storedManualTimeZone = [[NSUserDefaults standardUserDefaults]
								 boolForKey:TLTagalogManualTimeZoneDefault];
	NSInteger storedManualSecondsOffset = [[NSUserDefaults standardUserDefaults]
										   integerForKey:TLTagalogManualOffsetDefault];
...
static const double TLTagalog_multiplier = 60.0 * 60.0;
static const double TLTagalog_exponent = 2.5;

- (NSTimeInterval)errorSliderCameraError:(double)value {
	return TLTagalog_multiplier * copysign(pow(fabs(value), TLTagalog_exponent), value);
}

- (double)errorSliderValue:(NSTimeInterval)error {
	return copysign(pow(fabs(error / TLTagalog_multiplier), 1.0 / TLTagalog_exponent), error);
}
